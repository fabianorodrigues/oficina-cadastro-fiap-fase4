...
public class OficinaJsonConsoleFormatterTests
{
    ...
    public void Deve_emitir_os_campos_do_contrato_no_nivel_superior()
    {
        ...
        Assert.Equal("oficina-cadastro", json.GetProperty("service.name").GetString());
    ...
    {
        using var source = new ActivitySource("Oficina.Cadastro.Testes");
        using var listener = new ActivityListener
        {
            ShouldListenTo = candidate => candidate.Name == "Oficina.Cadastro.Testes",
...
public class OficinaTelemetryResourceTests
{
    ...
    {
        var resource = OficinaTelemetryResource.Resolve(Configuracao(), "oficina-cadastro");
...
public class CorrelationIdMiddlewareObservabilidadeTests
{
    ...
    {
        using var source = new ActivitySource("Oficina.Cadastro.Testes.Middleware");
        using var listener = new ActivityListener
        {
            ShouldListenTo = candidate => candidate.Name == "Oficina.Cadastro.Testes.Middleware",
            Sample = (ref ActivityCreationOptions<ActivityContext> _) => ActivitySamplingResult.AllDataAndRecorded
        };
        ActivitySource.AddActivityListener(listener);
