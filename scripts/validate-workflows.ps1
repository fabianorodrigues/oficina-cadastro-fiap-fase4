...
$contracts = @{
  'oficina-cadastro-fiap-fase4' = @{
    Ci = 'Cadastro CI'; Manuals = @{ 'Cadastro Deploy' = 'DEPLOY' }
  }
...
foreach ($rule in $forbiddenGlobal) {
...
foreach ($file in $files) {
  ...
  if ($name -eq $contract.Ci) {
    ...
    if (Has $text 'aws-actions/configure-aws-credentials|secrets\.AWS_ACCESS_KEY_ID|terraform\s+apply|kubectl\s+apply(?!\s+--dry-run=client)|helm\s+upgrade|docker\s+push|aws\s+(put|create|update|delete)-') {
      Add-Err "${name}: CI contem credencial AWS ou comando mutavel."
    }
  }
  elseif ($contract.Manuals.ContainsKey($name)) {
    ...
    Add-Err "$($file.Name): workflow nao esperado pelo contrato: $name"
  }
}
